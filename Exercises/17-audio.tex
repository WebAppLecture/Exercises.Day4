...
%
\par Erstellen Sie eine Webseite die ein \htag{audio}-Element, sowie vier
...
\item
Geben Sie die Quelle nicht über das \jvar{src}-Attribut an, sondern über
\htag{source}-Elemente.
\item
...
letzten Punkt zu verwirklichen müssen Sie das \jvar{onclick}-Ereignis der
...
Zustand.
\item
Mit den Methoden \jfunc{play} und \jfunc{pause} steuern Sie den aktuellen
Zustand. Die aktuelle Zeit setzen Sie über die Eigenschaft
\jvar{currentTime}\footnote{In einigen Browser ist das Setzen von
\jvar{currentTime} und \jvar{playbackRate} noch nicht implementiert.}.
\item
